...
(function( root, factory ) {
	...
	function configure( options ) {
		...
		if( numberOfSlides > 1 && config.autoSlideStoppable && features.canvas && features.requestAnimationFrame ) {
	...
	function speak(text) {
		...
		if (speaking || autoSlidePaused == true || !text || text.length == 0) {
			return;
		}
	...
	function step_through_lines(lines, local_indexh, local_indexv) {
		...
		if (!lines || lines.length == 0) {
			speaking = false;
			if( !autoSlidePaused && !isPaused() && !isOverview() && ( !Reveal.isLastSlide() || availableFragments().next || config.loop === true ) ) {
				autoSlideTimeout = setTimeout( navigateNext, 750 );
